...
| 拋體運動實驗室 | beta | `projectilemotion/projectilemotion.html` | 以互動式方格紙示範地表拋體運動、速度分量、飛行時間、水平位移X與海拔變化 |
...
- 2026-07-12：重整「拋體運動實驗室」為固定水平地表 `y = 0` 的拋體模型，移除斜面角輸入、繪製與交會計算。
- 2026-07-12：將拋體運動的右側面板整理為「解算結果」，加入 `00 基本公式`、6 張公式結果卡、解算按鈕、P1、V₀、Vx、Vy、動畫落點 T、縮放平移、預設比例、適合軌跡、右側參數控制區收合與向左全螢幕。
- 2026-07-12：建立並更新 `.codex/skills/extend-classroom-lab/references/non-regression-baselines.md`，作為後續工具維護的不可退步功能清單。
- 2026-07-11：將手動加入的 `ogo/` 原型重建為可直接由 GitHub Pages 執行的「軌道觀測站」，並加入工具總頁。
...
- 可調整初速、發射角與發射點高度 H（Y軸）；初速範圍為 5–500 m/s，高度範圍為 0–500 m，預設值為初速 50 m/s、發射角 45°、發射點高度 H = 0 m。
- 固定以 `y = 0` 表示水平地表；軌跡上的 `Y` 值與發射高度 `H` 可等效理解為垂直位移（海拔），`X` 表示水平位移X。
- Canvas 會即時顯示固定地表、發射點高度 H（Y軸）參考、發射點 P1、實線 V₀，以及由 P1 出發的淺藍色虛線 Vx 與紫色虛線 Vy；按下「解算」後，才以動畫繪製青色拋物線並在完成時標示地表落點 T。
- Canvas 支援滑鼠滾輪及畫面上的縮小、預設比例、適合軌跡、放大按鈕進行無段縮放，也可長按滑鼠左鍵拖曳平移；`1:1` 會恢復預設縮放倍率，`⤢` 則重新置中並容納目前軌跡。
- 「解算結果」最上方以兩倍卡片寬度靜態顯示 `00 基本公式`，列出三項基本運動公式；其下依序顯示 01–06 共 6 個結果卡：水平初速度、垂直初速度、飛行時間、抵達最高點時間、水平位移X與最高點垂直位移Y；第 08 格仍為「解算」按鈕。
- 六張結果卡均由 `v = v₀ + a‧t`、`x = v₀‧t + 1/2 a‧t²`、`v² = v₀² + 2‧a‧x` 三個基本運動公式搭配 `Vx = V₀‧cos α`、`Vy = V₀‧sin α` 推得；「操作說明」逐項列出軸向代入與對應卡片。
- 每張結果卡的符號、公式、即時代入參數、單位與最終數值一對一；代數式乘法統一使用置中的 `‧`、除法使用上下分式，並以高亮最終值，最高點統一使用 `最高點垂直位移Y（海拔） = H + Vy²/(2‧g)`。
- 桌面版可將右側參數控制區向右收合，讓左側解算展示區域使用完整頁面寬度；分界處收合按鈕上方另有同系列 `‹` 圖示的向左全螢幕按鈕。全螢幕模式會放大文字並採用 75% 不透明底色，讓左側解算展示區域仍隱約可見，按 `›` 或 Esc 可退出。
- 後續維護用語中，初速、發射角、發射點高度 H（Y軸）、解算結果、重設參數與示範組合所在區域稱為「右側參數控制區」；拋物線 Canvas 所在區域稱為「左側解算展示區域」。
...
1. 調整初速、發射角與發射點高度 H（Y軸）。
2. 對照 P1 的實線 V₀ 與虛線 Vx、Vy 方向，並查看右側「解算結果」中的逐卡公式與代入數值；調整初速時，V₀ 向量長度會同步改變。
3. 按下第 08 格「解算」，觀察青色拋物線動畫抵達 `y = 0` 後顯示落點 T。
4. 使用滑鼠滾輪或畫面按鈕無段縮放，按 `1:1` 恢復預設比例，長按滑鼠左鍵拖曳可平移畫面；需要時按「適合目前軌跡」恢復完整視野。
5. 桌面版可按左右區域分界箭頭，將右側參數控制區向右收合，或使用同系列箭頭切換向左全螢幕；全螢幕時可按 `›` 或 Esc 退出。
6. 開啟「公式與原理」查看模型假設與教學限制。
...
8. 更新本 README 的成果表、使用方式、技術架構、外部 API、專案結構與教學限制中受影響的段落。
...
- 拋體運動實驗室採用水平地表的理想模型，忽略空氣阻力、風、彈體旋轉、地球曲率與真實外彈道，只用於課堂說明等加速度運動。角度顯示一律使用度；`y = 0` 固定為地表，正 `Y` 值與 `H` 表示垂直位移（海拔），`X` 表示水平位移X。落地時間由 `0 = H + V₀‧sin α‧t − 4.9‧t²` 的非負時間根決定。
