...
- 2026-07-12：統一拋體運動命名與公式展示：發射點為 `P`，P 初速度為 `V_{P0}`，分量顯示為 `V_{0,x}`、`V_{0,y}`；右側面板改為「解算結果」，最上方為兩倍寬的 `00 基本公式`，下方固定保留 01–06 六張結果卡，第 07 格為「P軌跡解算」按鈕。
- 2026-07-12：完成拋體運動的互動行為歸結：按下「P軌跡解算」後才動畫繪製青色拋物線與落點 T；Canvas 支援滾輪縮放、按鈕縮放、`1:1` 預設比例、適合軌跡與滑鼠拖曳平移；`V_{P0}`、`V_{0,x}`、`V_{0,y}` 會跟隨初速與縮放比例改變可見長度。
- 2026-07-12：完成拋體運動版面與顯示控制歸結：右側參數控制區可向右收合，也可向左全螢幕並保留半透明背景；左側解算展示區域提供兩個等寬、等高且含狀態燈的緊湊顯示按鈕，用於切換 P 點所有向量及拋物線，或同步切換 `V_{0,x}`、`V_{0,y}`，且不重設參數、公式數值、座標視野或已解算狀態。
...
- 可調整 P 初速度 `V_{P0}`、發射角與發射點高度 H（Y軸）；初速範圍為 5–500 m/s，高度範圍為 0–500 m，預設值為初速 50 m/s、發射角 45°、發射點高度 H = 0 m。
- 固定以 `y = 0` 表示水平地表；軌跡上的 `Y` 值與發射高度 `H` 可等效理解為垂直位移（海拔），`X` 表示水平位移X。
- Canvas 會即時顯示固定地表、發射點高度 H（Y軸）參考、發射點 P、實線 `V_{P0}`，以及由 P 出發的淺藍色虛線 `V_{0,x}` 與紫色虛線 `V_{0,y}`；按下「P軌跡解算」後，才以動畫繪製青色拋物線並在完成時標示地表落點 T。
- 左側解算展示區域提供兩個等寬、等高且含狀態燈的緊湊線段顯示按鈕：「顯示 P 點所有向量及拋物線」整批控制 `V_{P0}`、`V_{0,x}`、`V_{0,y}` 與拋物線；「顯示 V_{P0}分量」同時控制 `V_{0,x}`、`V_{0,y}`。P、T、座標視野、參數與解算數據不受切換影響。
- Canvas 支援滑鼠滾輪及畫面上的縮小、預設比例、適合軌跡、放大按鈕進行無段縮放，也可長按滑鼠左鍵拖曳平移；`V_{P0}`、`V_{0,x}`、`V_{0,y}` 會與底圖及拋物線同步縮放，`1:1` 會恢復預設縮放倍率，`⤢` 則重新置中並容納目前軌跡。
- 「解算結果」最上方以兩倍卡片寬度靜態顯示 `00 基本公式`，列出三項基本運動公式；其下依序顯示 01–06 共 6 個結果卡：`V_{0,x}`分量、`V_{0,y}`分量、飛行時間、抵達最高點時間、水平位移X與最高點垂直位移Y；第 07 格為「P軌跡解算」按鈕。
- 六張結果卡均由 `v = v₀ + a‧t`、`x = v₀‧t + 1/2 a‧t²`、`v² = v₀² + 2‧a‧x` 三個基本運動公式搭配 `V_{0,x} = V_{P0}‧cos α`、`V_{0,y} = V_{P0}‧sin α` 推得；「操作說明」逐項列出軸向代入與對應卡片。
- 每張結果卡的符號、公式、即時代入參數、單位與最終數值一對一；代數式乘法統一使用置中的 `‧`、除法使用上下分式，並以高亮最終值，最高點統一使用 `最高點垂直位移Y（海拔） = H + V_{0,y}²/(2‧g)`。
- 桌面版可將右側參數控制區向右收合，讓左側解算展示區域使用完整頁面寬度；分界處收合按鈕上方另有同系列 `‹` 圖示的向左全螢幕按鈕。全螢幕模式會放大文字並採用 75% 不透明底色，讓左側解算展示區域仍隱約可見，按 `›` 或 Esc 可退出。
- 後續維護用語中，P 初速度、發射角、發射點高度 H（Y軸）、解算結果、重設參數與示範組合所在區域稱為「右側參數控制區」；拋物線 Canvas 與兩個線段顯示按鈕所在區域稱為「左側解算展示區域」。
...
1. 調整 P 初速度 `V_{P0}`、發射角與發射點高度 H（Y軸）。
2. 對照 P 的實線 `V_{P0}` 與虛線 `V_{0,x}`、`V_{0,y}` 方向，並查看右側「解算結果」中的逐卡公式與代入數值；調整初速時，`V_{P0}` 向量長度會同步改變。
3. 按下第 07 格「P軌跡解算」，觀察青色拋物線動畫抵達 `y = 0` 後顯示落點 T。
4. 使用滑鼠滾輪或畫面按鈕無段縮放，觀察 `V_{P0}`、`V_{0,x}`、`V_{0,y}` 與軌跡同步改變長度；按 `1:1` 恢復預設比例，長按滑鼠左鍵拖曳可平移畫面，需要時按「適合目前軌跡」恢復完整視野。
5. 使用左側解算展示區域下方的兩個按鈕，整批切換 P 點向量與拋物線，或以「顯示 V_{P0}分量」同步切換 `V_{0,x}`、`V_{0,y}`；確認參數與解算結果保持不變。
6. 桌面版可按左右區域分界箭頭，將右側參數控制區向右收合，或使用同系列箭頭切換向左全螢幕；全螢幕時可按 `›` 或 Esc 退出。
7. 開啟「公式與原理」查看模型假設與教學限制。
...
- 拋體運動實驗室採用水平地表的理想模型，忽略空氣阻力、風、彈體旋轉、地球曲率與真實外彈道，只用於課堂說明等加速度運動。角度顯示一律使用度；`y = 0` 固定為地表，正 `Y` 值與 `H` 表示垂直位移（海拔），`X` 表示水平位移X。落地時間由 `0 = H + V_{P0}‧sin α‧t − 4.9‧t²` 的非負時間根決定。
