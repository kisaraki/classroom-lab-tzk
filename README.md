...
| 三角函數實驗室 | 可使用 | `trigfun/trigfun.html` | 以單位圓、Magic Hexagon 與六個週期波形探索廣義角及六種三角函數 |
...
GPS 頁面會從 CDN 載入 Three.js、OrbitControls 與地球紋理，因此完整使用時需要網路連線。拋體運動、透鏡成像、數字系統與三角函數頁面不依賴外部 CDN。
