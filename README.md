...
- 以桌面瀏覽器第一人稱血液循環賽車呈現四關路線，依序涵蓋腹部及下肢體循環、肺循環、頭部胸部及上肢體循環，以及高危險肺循環。
...
- 需要桌面版 Chrome、Edge 或 Firefox，以及鍵盤、滑鼠、WebGL、ES Modules 與 Pointer Lock；最低驗收解析度為 1280 × 720。手機與平板會在載入 Three.js 前顯示不支援提示。
...
1. 使用支援 WebGL、ES Modules 與 Pointer Lock 的桌面瀏覽器，以至少 1280 × 720 的畫面開啟工具。
2. 按「開始遊戲並鎖定滑鼠視角」進入第一關；若拒絕或瀏覽器不支援 Pointer Lock，依畫面提示重新嘗試。
3. 使用方向鍵在血管截面移動紅血球，按 `Z`／`X` 提高或降低 BP，移動滑鼠改變視線；WASD、滑鼠按鍵與滾輪不控制紅血球。
4. 進入氣體交換時，依畫面要求按 `O` 與 `C` 完成氧氣及二氧化碳輸入，並對照循環圖與紅血球顏色變化。
