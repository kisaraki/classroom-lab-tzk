...
| 拋體運動實驗室 | beta | `projectilemotion/projectilemotion.html` | 以共用互動式方格紙比較 P、Q 兩組地表拋體運動、速度分量、飛行時間、水平位移X與海拔變化 |
...
- 2026-07-14：拋體運動實驗室新增第二發射點 Q。P、Q 使用並列參數與並列結果，可在同一 Canvas 各自解算並重疊保留圖形；八條向量與軌跡採互不重複的黃色、淺藍、冰白、青色、紫色、橙色、亮綠與大紅色，Q 軌跡加入雙層描線微光，01–06 結果卡文字放大兩級，並新增 Q 的兩個顯示開關及第 08 格「Q軌跡解算」按鈕。
- 2026-07-12：歸結本輪「拋體運動實驗室」改版結果，確立為固定水平地表 `y = 0` 的理想拋體模型；已移除斜面角輸入、斜面繪製、斜面交會計算與相關敘述，後續以 `X` 表示水平位移X、以正 `Y` 與 `H` 表示垂直位移（海拔）。
- 2026-07-12：完成拋體運動實驗室的互動與版面基礎：右側改為含 `00 基本公式`、01–06 結果卡及軌跡解算按鈕的「解算結果」，左側 Canvas 支援滾輪與按鈕無段縮放、`1:1` 預設比例、適合軌跡及滑鼠拖曳平移；右側參數控制區可向右收合或向左全螢幕，線段顯示開關只控制可見性，不重設參數、公式、視野或解算狀態。上述結構於 2026-07-14 擴充為目前的 P、Q 雙發射點版本。
